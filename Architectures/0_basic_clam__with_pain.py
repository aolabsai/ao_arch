...
arch_c = [2]           # adding 2 control neuron which we'll define with the instinct control function below
...
# Adding Instinct Control Neuron - PLEASURE (Positive Reinforcement)
c0 = 3  # set-relative id of neuron; the first 3 neurons (ids 0, 1, 2) are included in all Archs by default (corresponding to LABEL, Cpos, Cneg)
...
# Labelling the neuron as a Pleasure neuron for ao_core digestion
Arch.C__flat_pleasure = np.append(Arch.C__flat_pleasure, Arch.C__flat[c0])


# Adding Instinct Control Neuron - PAIN (Negative Reinforcement)
c1 = 4 # set-relative id of neuron
def c1_instinct_rule(INPUT, Agent):
    # 
    # Define instinct logic here
    #       
    return
# Saving the function to the Arch so the Agent can access it
Arch.datamatrix[4, Arch.C[1][1]] = c1_instinct_rule
# Labelling the neuron as a Pain neuron for ao_core digestion
